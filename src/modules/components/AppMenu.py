...
def setTheme(ctk, options):
    ...
    store.setMenuOption("theme", options)
...
def create(ctk, master, state):
    ...
    # ----- Standalone options ----- #
    options_submenu.add_checkbutton(label="Auto Close", variable=options["autoclose"], onvalue=True, offvalue=False, command=lambda: store.setMenuOption("autoclose", options))
    options_submenu.add_checkbutton(label="Debug", variable=options["debug"], onvalue=True, offvalue=False, command=lambda: store.setMenuOption("debug", options))
    options_submenu.add_checkbutton(label="Logs", variable=options["logging"], onvalue=True, offvalue=False, command=lambda: store.setMenuOption("logging", options))
